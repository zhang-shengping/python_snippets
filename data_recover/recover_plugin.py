...
class LoadBalancerPluginv2(loadbalancerv2.LoadBalancerPluginBaseV2,
    ...
    def create_loadbalancer(self, context, lb_id):
        lb_db = self.db.get_loadbalancer(context, lb_id)
        loadbalancer = lb_db.to_api_dict()
    ...
    def create_listener(self, context, ls_id):
        listener_db = self.db.get_listener(context, ls_id)
        listener = listener_db.to_api_dict()
        ls_lb = listener.get('loadbalancers')
        if ls_lb:
            lb_id = ls_lb[0]["id"]
    ...
    def create_pool(self, context, pl_id):
        db_pool = self.db.get_pool(context, pl_id)
        pool = db_pool.to_api_dict()
        ...
        for l in listeners:
            ...
            if db_l.loadbalancer.id != lb_id:
                raise sharedpools.ListenerAndPoolMustBeOnSameLoadbalancer()
            import pdb; pdb.set_trace()
            if db_l.default_pool_id and db_l.default_pool_id != pl_id:
    ...
    def create_pool_member(self, context, pool_id, mb_id):
        member_db = self.db.get_pool_member(context, mb_id)
        member = member_db.to_api_dict()
        import pdb; pdb.set_trace()
